...
$target_dir = "../uploaded_student_profile_photo/";
...
if (isset($_FILES['profile_pic_file'])) {
    ...
    if (move_uploaded_file($_FILES["profile_pic_file"]["tmp_name"], $target_file)) {
        $profile_pic_image_name = "http://localhost/project/uploaded_student_profile_photo/" . $email . "_" . $profile_pic_name;
        $update_fields .= "sp_profile_image = '$profile_pic_image_name', ";
    }
}
...
if (!empty($update_fields)) {
    $sql = "UPDATE stud_profile SET $update_fields WHERE sp_email = '$email'";
